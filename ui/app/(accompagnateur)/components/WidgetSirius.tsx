...
export function WidgetSirius({ params, fallbackComponent }: { fallbackComponent?: JSX.Element; params: string }) {
  const [isNotFound, setIsNotFound] = useState(false);
  const [height, setHeight] = useState(0);
  const API_URL = process.env.NEXT_PUBLIC_SIRIUS_API_BASE_URL || "https://sirius.inserjeunes.beta.gouv.fr";

  const ref = useRef<HTMLDivElement>(null);
  const widgetCode = `<iframe style="width: 100%; height: 0px; margin-bottom: 2rem;" src="${API_URL}/iframes/${params}"
